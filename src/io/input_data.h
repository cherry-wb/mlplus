...
public:
     ...
     * \date 12/11/2005
     */
    InputData() : _hasExampleName(false), _subsetAlreadyComputed(false), _classInLastColumn(false), _numExamples(0)
    {
        _pData = new RawData();
    }
     ...
     * \date 10/11/2005
     */
    inline const vector<Label>& getLabels(const int idx) const
    {
        return _pData->getLabels(_indirectIndices[idx]);
    }
    inline       vector<Label>& getLabels(const int idx)
    {
        return _pData->getLabels(_indirectIndices[idx]);
    }
     ...
     * \date 14/2/2006
     */
    const string& getExampleName(const int idx)
    {
        return _pData->getExampleName(_indirectIndices[idx]);
    }

    int      getNumAttributes()  const
    {
        return _pData->getNumAttributes();    //!< Returns the number of attributes.
    }
    int      getNumExamples()    const
    {
        return _numExamples;    //!< Returns the number of examples.
    }
    int      getNumClasses()    const
    {
        return _pData->getNumClasses();    //!< Returns the number of classes.
    }
    ...
    {
        return _numExamples != _pData->getNumExample();
    }
    //TODO: comment
    inline int getRawIndex(int i)
    {
        return _indirectIndices[i];
    }
    //TODO: comment
    inline bool isUsedIndice(int x)
    {
        return _rawIndices[x] > -1;
    }

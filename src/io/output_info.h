...
public:
    ...
    void outputHeader(ostream& outStream, const NameMap& namemap)
    {
        outStream <<  "mae" << OUTPUT_SEPARATOR << HEADER_FIELD_LENGTH << "mse";
    }
...
public:
    ...
    void outputHeader(ostream& outStream, const NameMap& namemap)
    {
        outStream << "minmargin" << OUTPUT_SEPARATOR << HEADER_FIELD_LENGTH << "negmargins" ;
    }
...
/**
 * Output for the LHCb data
 * Each Event can have multiple candidates. If one candidate of an event is classified
 * positive, then, the corresponding event is classified positive.
 * \date 14/08/2011
 */
class LHCbOutput : public BaseOutputInfoType
{

public:


    LHCbOutput(const nor_utils::Args& args)
    {
        string rawDataFileName;

        if(args.hasArgument("rawdata"))
        {
            args.getValue("rawdata", 0, rawDataFileName);
            _rawDataFile.open(rawDataFileName.c_str());
            assert(_rawDataFile.good());
        }
        else
        {
            cout << "Error : The information about the event/candidates is missing, please provide the right file with --rawdata.\n";
            exit(-1);
        }
    }
...
protected:
    ...
    int getNextCandidatesNumber();

    ifstream _rawDataFile;

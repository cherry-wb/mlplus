...
BaseOutputInfoType* BaseOutputInfoType::createOutput(string type, const nor_utils::Args* args)
{
    ...
    if(type.compare("lhc") == 0) return new LHCbOutput(*args);
...
void LHCbOutput::computeAndOutput(ostream& outStream, InputData* pData,
                                  map<InputData*, table>& gTableMap,
                                  map<InputData*, table>& marginsTableMap,
                                  map<InputData*, AlphaReal>& alphaSums,
                                  BaseLearner* pWeakHypothesis)
{
    ...
    const NameMap& namemap = pData->getClassMap();
    int positiveLabelIndex = namemap.getIdxFromName("sgnl");
    ...
    while(i < numExamples)
    {
        ...
        int numCandidates = getNextCandidatesNumber();
        assert(numCandidates > 0);

        const vector<Label>& eventlabels = pData->getLabels(i);
        int eventForecast = -1;

        for(int j = 0; j < numCandidates; ++j, ++i)
        {

            const vector<Label>& candidatelabels = pData->getLabels(i);

            // just a small check that the raw data file is well read.
            assert(eventlabels[positiveLabelIndex].y == candidatelabels[positiveLabelIndex].y);

            if(g[i][positiveLabelIndex] > 0)
            {
                eventForecast = +1;
            }
        }

        if(eventForecast * eventlabels[positiveLabelIndex].y < 0)
        {
            numErrors++;
        }
    ...
    // The error is normalized by the number of points
    outStream  << (AlphaReal)(numErrors) / (AlphaReal)(eventNumber);

}

// -------------------------------------------------------------------------

int LHCbOutput::getNextCandidatesNumber()
{
    int num = -1;
    if(!_rawDataFile.eof())
    {
